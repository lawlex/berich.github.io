...
var Deck = (function () {
  ...
  // holdem area
  var count_players; // ten max
  var hand = []; // two cards
  var player = []; // ten max
  var flop = []; // three cards
  var turn; // one card
  var river; // one card
  // end holdem area
  ...
  function _card(i) {
    ...
    // self = card
    var self = { i: i, rank: rank, suit: suit, pos: i, $el: $el, mount: mount, unmount: unmount, setSide: setSide, textCode: textcode, colorHuman: color_human, humanRankSuit: rank_human+suit_human, color4: color4};
  ...
  var moveto = {
    deck: function deck(_deck8) {
      _deck8.moveto = _deck8.queued(moveto);

      function moveto(next, reverse, x, y, rot, step) {
        ...
        cards.forEach(function (card, i) {
          card.moveto(i, cards.length, function (i) {
            if (i === cards.length - 1) {
              next();
            }
          }, reverse, x, y, rot, step);
        });
      }
    },
    card: function card(_card8) {
      ...
      _card8.moveto = function (i, len, cb, reverse, x=0, y=0, rot=0, step=1) {
        var z = i / 4;
        var delay = i * 10;
    ...
    card: function card(_card7) {
      ...
      _card7.bysuit = function (cb) {
        ...
        _card7.animateTo({
          ...
          y: Math.round((2.5 - suit) * 92 * ___fontSize / 16),
          rot: 0,
  ...
  Deck.ease = ease;
  Deck.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, shuffle: shuffle, sort: sort, flip: flip, moveto: moveto };
  Deck.Card = _card;
  Deck.prefix = prefix;
